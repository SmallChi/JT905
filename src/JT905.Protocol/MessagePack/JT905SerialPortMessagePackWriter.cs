...
namespace JT905.Protocol.MessagePack
{
    ...
    /// JT905消息写入器
    /// </summary>
    public ref struct JT905SerialPortMessagePackWriter
    {
        ...
        public JT905SerialPortMessagePackWriter(Span<byte> buffer, JT905Version version = JT905Version.JTT2014)
        {
        ...
        /// <summary>
        /// 写入七个字节的日期类型,yyyyMMddHHmmss
        /// </summary>
        /// <param name="value"></param>
        /// <param name="fromBase"></param>
        public void WriteDateTime7(in DateTime value, in int fromBase = 16)
        {
            var span = writer.Free;
            var s = value.ToString("yyyy");
            span[0] = Convert.ToByte(s[..2], fromBase);
            span[1] = Convert.ToByte(s[2..], fromBase);
            ...
            span[6] = Convert.ToByte(value.ToString("ss"), fromBase);
            writer.Advance(7);
        }
        /// <summary>
        ...
        /// 将内存块进行或运算并写入一个字节
        /// </summary>
        public void WriteXor(int start = 1)
        {
            ...
            //从指定位置开始
            var xorSpan = writer.Written[start..];
            byte result = xorSpan[0];
        ...
        /// <param name="value"></param>
        /// <param name="len"></param>
        public void WriteHex(string value, in int len=-1)
        {
            value ??= "";
            value = value.Replace(" ", "");
            int startIndex = 0;
        ...
        public void WriteASCII(in string value, int padLength, byte padValue = 0)
        {
            var spanFree = writer.Free;
            var bytes = Encoding.ASCII.GetBytes(value).AsSpan();
            WriteArray(bytes);
            for (int i = 0; i < padLength - bytes.Length; i++)
            {
                WriteByte(padValue);
            }
        }
