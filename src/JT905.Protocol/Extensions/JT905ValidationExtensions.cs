...
namespace JT905.Protocol.Extensions
{
    ...
    /// 验证长度扩展方法
    /// </summary>
    public static partial class JT905ValidationExtensions
    {
        /// <summary>
        /// 验证字符串长度
        /// </summary>
        ...
        public static string ValiMaxString(this string value, in string fieldName, in int maxLength)
        {
            ...
            {
                throw new JT905Exception(JT905ErrorCode.VailLength, $"{fieldName}:{value.Length}>max length[{maxLength}]");
            }
            return value;
        }
        ...
        private static void vali(in int length,in string fieldName, in int fixedLength)
        {
            ...
            {
                throw new JT905Exception(JT905ErrorCode.VailLength, $"{fieldName}:{length}>fixed[{fixedLength}]");
            }
            else if (length < fixedLength)
            {
                throw new JT905Exception(JT905ErrorCode.NotEnoughLength, $"{fieldName}:{length}<fixed[{fixedLength}]");
            }
        }
    }
}
